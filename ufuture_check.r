...
#prefuture = function(res_vpa, res_SR_MSY, fyear){#チェック時以外はコメントアウトを外す
  # 中の計算を確認するための入力
...
#  return(prelist)
#}#prefuture終わり#チェック時以外はコメントアウトを外す


#2-2. ufuture: Fを与え、rep回の施行 を実行する関数----

#ufuture = function(prepare,rep,fbeta){#チェック時以外はコメントアウトを外す
  ...
  #} #試行回数のループ終わり   #チェック時以外はコメントアウトを外す、これ以下の行も同様。
